var FisheyeGl = function FisheyeGl(options) {
  ...
  // Remove require and use a simple shader object instead
  var shaders = {
    vertex: `
...
void main(void){
  ...
  var vertexSrc = options.vertexSrc
    ? loadFile(options.vertexSrc)
    : shaders.vertex;
  var fragmentSrc = options.fragmentSrc
    ? loadFile(options.fragmentSrc)
    : shaders.fragment3;
